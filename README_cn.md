...
**AI 整理书签** *(v0.1.0+ 新功能)*
* 一键把所有书签用大模型**自动分类**（最多 10 大类，LLM 根据 URL/标题/域名自动创建分类名）
* LLM 遵循 6 条规则：大类聚合（所有加密货币→1 类、所有 AI→1 类、所有视频→1 类…）+ 分类名清理
* 实时进度弹窗，带"任务卡住？重置"逃生口
* 整理结果写入 Gist 顶级新增的 `AI分类` 根目录
...
* 🤖 **AI 整理书签** —— 一键用大模型把全部书签**自动分类**（详见 [使用方法 → AI 整理书签](#ai-整理书签新)）。
...
* LLM 根据 URL/标题/域名**自动创建分类名**（不再是固定 10 大类）
* 结果写回 Gist 顶级新增的 `AI分类` 根目录，每个分类一个子文件夹
...
如果 LLM 分类结果超过 10 个，**数量最少**的几个会被合并到「其他」（修复了之前保留最少合并最多的反向 bug）。
